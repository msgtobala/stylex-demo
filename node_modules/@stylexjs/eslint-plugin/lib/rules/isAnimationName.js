...
function isAnimationName(node, variables) {
  if (node.type === 'CallExpression' && node.callee.type === 'MemberExpression' && node.callee.object.type === 'Identifier' && node.callee.object.name === 'stylex' && node.callee.property.type === 'Identifier' && node.callee.property.name === 'keyframes') {
    return undefined;
  }
  ...
  if (node.type === 'TemplateLiteral') {
    if (!node.expressions.every(expr => isAnimationName(expr, variables) === undefined)) {
      return {
        message: 'All expressions in a template literal must be a `stylex.keyframes(...)` function call'
      };
    }
